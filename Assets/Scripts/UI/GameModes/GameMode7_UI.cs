    ...
    public class GameMode7_UI : GameModeUI
    {
        ...
        protected override void Update()
        {
            ...
            if (PlayerController.Instance.State == PlayerState.Prey)
            {
                ...
                {
                    if (Input.GetKeyDown(KeyCode.E) && (GameMode.Instance as GameMode7).SwitchCooldownTimer > 0)
                    {
                        switchText.transform.DOKill();
                        (switchText.transform as RectTransform).DOShakeAnchorPos(.5f, 20).OnComplete(() => { (switchText.transform as RectTransform).anchoredPosition = switchTextOriginalPosition; });
